...
<form method="post" action="/getSimilarDocuments/">
  ...
  <input type="hidden" name="submit" value="find similar documents!" />
  <input type="submit" value="find similar documents!" />
</form>
<br /><br /><br />
<table py:if="error == 0" align="center">
  <tr>
    <td width="750">
      <h1>Similar documents for <span py:if="url != 'text'">'<a href="${url}" target="_blank">${url}</a>'</span><span py:if="url == 'text'">${url}</span></h1><br />
      <table width="100%">
	<tr>
	  <td class="maintext">
	    <table>
	      <tr py:for="document in similarDocuments">
		<td>
		  <a href="${document.URL}" target="_blank"><span py:content="XML(document.title)">X</span></a> &rArr; <a href="${tg.url('/getSimilarDocuments/?url=' + document.URL)}">Find similar documents</a> &rArr; <a href="${tg.url('/process/?url=' + document.URL + '&amp;language=automatic')}">Topicalize</a><br />
		</td>
	      </tr>
	    </table>
	  </td>
