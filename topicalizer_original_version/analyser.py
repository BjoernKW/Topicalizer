...
class WordStructure:
    ...
    # get types, characters and sentences from tokens
    def getTypesCharactersSentences(self, tokens, minWordsPerSentence = 2):
	...
	# syllables = re.compile(r'[BCDFGHJKLMNPQRSTVWXYZbcdfghjklmnpqrstvwxyz]{1}[\&AEIOUÄÖÜYaeiouäöüy]|^[\&AEIOUÄÖÜYaeiouäöüy]')
	syllables = re.compile(r'[\;AEIOUÄÖÜYaeiouäöüy][BCDFGHJKLMNPQRSTVWXYZbcdfghjklmnpqrstvwxyz]')
    ...
    # function for retrieving n-gram structure from corpus
    def getNGramStructure (self, corpus):
    ...
    # function for comparing n-gram structure of corpus with language profiles
    def compareNGramStructure (self, ngramStructure):
	# initialise minimum distance
	minimumDistance = 1001

	# initialise corpus language
	corpusLanguage = ''
	...
	# go through each language profile and compare it to n-gram structure
	for language in languages:
	    ...
	    # open profile file
	    profileFile = codecs.open('topicalizer/languageprofiles/' + language + '.txt', 'r', 'iso-8859-1')

	    # go through each line of file and write it to profile
	    ...
	    # go through each n-gram in n-gram structure
	    for ngram, ngramValue in sorted(ngramStructure.iteritems(), key = itemgetter(1), reverse = True):
		# go through each ngram in lanugage profile
		for j, profileNGram in enumerate(profile):
		    if ngram == profileNGram:
			distance += fabs(i - j)

		# increment counter
		i += 1

	# if distance is smaller than current minimum distance, set corpus language to current language
...
class SentenceStructure:
    ...
    # get co-occurrences for words in sentence using a pickled hash
    def getCoOccurrences (self, stopWordList, tokens, textCategory):
	# initialise keyword list
	keywords = []

	# set path
	path = './topicalizer/corpora/brown/'

	# open file
	file = open(path + textCategory + '.co.dump', 'r')
	...
	for token in tokens:
	    # get five most frequent co-occurrences for this token
	    try:
		thisCoOccurrences = coOccurrences[token]
		mostFrequentCoOccurrences = sorted(thisCoOccurrences.iteritems(), key = itemgetter(1), reverse = True)[:10]

		# get coOccurrences for each token
		for coOccurrence in mostFrequentCoOccurrences:
		    keywords.append(coOccurrence[0])
	    except KeyError:
		pass
